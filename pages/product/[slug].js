        ...
        <Wrapper>
          <div className="flex flex-col lg:flex-row md:px-10 gap-[50px] lg:gap-[100px]">
            ...
            <div className="w-full md:w-auto flex-[1.5] max-w-[500px] lg:max-w-full mx-auto lg:mx-0">
              <ProductDetailsCarousel images={p.images} name={p.name} />
            </div>
            {/* left column end */}
            ...
            <div className="flex-[1] py-3">
              ...
              <div className="mb-10">
                ...
                <div id="sizesGrid" className="grid grid-cols-3 gap-2">
                  {p?.size?.data?.map((psize, i) => (
                    <div
                      onClick={() => {
                        setSelectedSize(psize.size);
                        setShowError(false);
                      }}
                      key={i}
                      className={`border rounded-md text-center py-3 font-medium ${psize.enabled ? "hover:border-black cursor-pointer" : "cursor-not-allowed bg-black/[0.1]"} ${
                        selectedSize === psize.size ? "border-black" : ""
                      }`}
                    >
                      {psize.size}
                    </div>
              ...
              <button
                ...
                onClick={() => {
                  if (!selectedSize) {
                    ...
                    dispatch(addToCart({ ...product?.[0], selectedSize, oneQuantityPrice: p.price }));
                    notify();
                  }
                }}
              >
...
export async function getStaticPaths() {
  const products = await fetchDataFromApi("api/product/get-products");
...
export async function getStaticProps({ params: { slug } }) {
  ...
  const products = await fetchDataFromApi(`api/product/get-related-products?slug=${slug}`);
